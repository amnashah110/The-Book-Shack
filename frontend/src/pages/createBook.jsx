...
const CreateBook = () => {
    ...
    const saveBook = () => {
        ...
        axios
            ...
            .then(() => {
                setLoading(false);
                enqueueSnackbar('Book Added Successfully', { variant: 'success', autoHideDuration: 2000 });
                navigate('/');
            })
            .catch((error) => {
                setLoading(false);
                enqueueSnackbar('Error Adding Book', { variant: 'error', autoHideDuration: 2000 });
                console.log(error);
            });
    }
    return (
        <div className='fixed top-0 left-0 right-0 bottom-0 p-20 overflow-y-auto h-screen' style={{ position: 'relative', backgroundImage: 'url("https://wallpapercave.com/wp/wp10933264.jpg")', backgroundPosition: 'center' }}>
            ...
            <div className="flex flex-col border-4 w-[600px] p-4 mx-auto" style={{ backgroundColor: '#494949', marginTop: '3em', borderRadius: '2em' }}>
                ...
                <button className="p-2 bg-gray-500 m-8" onClick={saveBook} style={{ fontFamily: 'Poppins', fontSize: '1.8em', color: '#ffffff' }}>Save Book Details</button>
            </div>
        </div>
    )
}
